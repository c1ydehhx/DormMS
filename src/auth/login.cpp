...
void LoginPortal::login(){
    ...
    while(true){
        ...
        optional<Student> optional_student = StudentManageSystem::get_instance().get_student(account);
        ...
        if(optional_student.has_value() && optional_student->authenticate(password)){
            std::cout << "Login Success." << std::endl;
            std::cout << std::endl;
            break;
        }
