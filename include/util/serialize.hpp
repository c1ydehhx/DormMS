...
inline void serialize(std::vector<Student>& students, std::string path){
    ...
    for(Student student : students){
        student_data.push_back({student.get_student_id().to_string(), student.get_name(), student.get_password(), student.get_gender()});
    }
...
inline std::vector<Student> deserialize(std::string path){
    ...
    std::vector<StudentData> student_data;
    bitsery::Deserializer<bitsery::InputBufferAdapter<std::vector<uint8_t>>> des(buffer.begin(), buffer.end());
    des.container(student_data, 1000);

    std::vector<Student> students;

    for(StudentData data : student_data){
        students.push_back(Student(StudentID(data.student_id), data.name, data.gender, data.password));
    }
